...
class ClientManager(commands.Cog):
    ...
    async def check_user_roles(self, user_id, check_type="both"):
        ...
        try:
            ...
            logging.error("Database connection error on check_user_roles - aborting checking user roles")
            return None
        ...
        if check_type in ["rank", "both"]:
            ...
            if not correct_rank or rank_roles_count > 1:
                logging.debug(f"Setting rank for user {user_id} to {rank} (had {rank_roles_count} rank roles)")
        ...
        if check_type in ["division", "both"]:
            ...
            if not correct_division or division_roles_count > 1:
                logging.debug(f"Setting division for user {user_id} to {division} (had {division_roles_count} division roles)")
    ...
    async def scan_voice_channels(self):
        ...
        logging.debug(f"Initial voice channel scan complete. Found {len(self.connected_users)} users.")
