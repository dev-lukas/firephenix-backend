...
the actual SQL in DatabaseManager.capture_myteamspeak_id / find_user_by_
myteamspeak_id / merge_teamspeak_identity / recognize_teamspeak_client.
"""
...
class IdentityBridgeTests(unittest.TestCase):
    ...
    def test_capture_backfills_myteamspeak_id(self):
        ...
        updated = self.db.capture_myteamspeak_id("TS3-UID", "MYTSID-ALICE")
    ...
    def test_capture_noop_when_empty(self):
        ...
        self.assertFalse(self.db.capture_myteamspeak_id("TS3-UID", None))
        self.assertFalse(self.db.capture_myteamspeak_id("TS3-UID", ""))

    def test_capture_idempotent(self):
        seed_user(self.db, teamspeak_id="TS3-UID", name="Alice")
        self.assertTrue(self.db.capture_myteamspeak_id("TS3-UID", "MYTSID"))
        # already stored -> no row changed
        self.assertFalse(self.db.capture_myteamspeak_id("TS3-UID", "MYTSID"))

    # --- lookup --------------------------------------------------------- #
    def test_find_excludes_connecting_uid(self):
        uid = seed_user(self.db, teamspeak_id="OLD", name="Bob")
        self.db.capture_myteamspeak_id("OLD", "MYTSID-BOB")
        # Connecting with the same UID -> no *other* identity to bridge.
        self.assertIsNone(self.db.find_user_by_myteamspeak_id("MYTSID-BOB", exclude_uid="OLD"))
        # Connecting with a new UID -> finds the prior row.
        found = self.db.find_user_by_myteamspeak_id("MYTSID-BOB", exclude_uid="NEW")
    ...
    def test_recognize_merges_new_identity_into_history(self):
        ...
        self.db.capture_myteamspeak_id("OLD-UID", "MYTSID-CAROL")
        ...
        result = self.db.recognize_teamspeak_client("NEW-UID", "MYTSID-CAROL", is_ts6=False)
    ...
    def test_recognize_ts6_records_alias_and_keeps_ts3_id(self):
        ...
        self.db.capture_myteamspeak_id("TS3-SHA1", "MYTSID-DAVE")
        seed_time(self.db, platform="teamspeak", platform_uid="TS3-SHA1", total_time=500)

        result = self.db.recognize_teamspeak_client("TS6-SHA256", "MYTSID-DAVE", is_ts6=True)
    ...
    def test_recognize_noop_for_unknown_account(self):
        ...
        result = self.db.recognize_teamspeak_client("SOLO", "MYTSID-EVE", is_ts6=False)
    ...
    def test_merge_refuses_cross_account(self):
        ...
        result = self.db.merge_teamspeak_identity("CANON", "OTHER")
